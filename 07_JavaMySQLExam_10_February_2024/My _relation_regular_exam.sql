...
DELIMITER $$
CREATE FUNCTION udf_average_salary_by_position_name (positionName VARCHAR(40))
...
BEGIN
    ...
    SET position_average_salary := (
        ...
        WHERE positionName = p.name);
RETURN position_average_salary;
END$$


#11. Improving the standard of living
CREATE PROCEDURE udp_increase_salaries_by_country (countryName VARCHAR(50))
BEGIN
	UPDATE workers AS w
    JOIN preserves AS p ON w.preserve_id = p.id
	JOIN countries_preserves AS cp ON cp.preserve_id = p.id
